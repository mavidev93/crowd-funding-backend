    ...
    : describe("CrowdFund unit test", () => {
          ...
          describe("createCampaign", () => {
              ...
              it("revert when Days is lower than minimum(1)",async ()=>{
                ...
                ).to.be.revertedWith("CrowdFund__PeriodDaysShouldBeBetween1to365()")
              })

              it("emit Create event ",async()=>{
                await expect(crowdFund.createCampaign(HASH,MINIMUM_GOAL_AMOUNT,ACCEPTABLE_DAYS)).to.emit(crowdFund,"Create")
              })

              it("check fresh campaign data",async()=>{
                const tx =await crowdFund.createCampaign(HASH,MINIMUM_GOAL_AMOUNT,ACCEPTABLE_DAYS)
                tx.wait(1)
                const campaign = await crowdFund.getCampaignByHash(HA)
              })

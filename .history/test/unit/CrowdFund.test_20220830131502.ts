          ...
          describe("fund campaign", () => {
              ...
              it("store the funder with the value",async()=>{
                ...
                await crowdFundFunder.fundCampaign(HASH,{value:MINIMUM_FUND_AMOUNT})
                 const fundedFromContract = await crowdFundFunder.getFundedAmountByFunder(HASH,funder.address)
                  log(fundedFromContract.toNumber())

                  assert.equal(MINIMUM_FUND_AMOUNT,fundedFromContract.toNumber())

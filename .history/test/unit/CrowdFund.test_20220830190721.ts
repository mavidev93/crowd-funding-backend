...
!developmentChains.includes(network.name)
    ...
    : describe("CrowdFund unit test", () => {
        ...
        describe("get contributions",( )=>{
            it("get contribution amount right",async()=>{
                const createTx = await createCampaign()
               await crowdFundFunder.fundCampaign(HASH,{value:MINIMUM_FUND_AMOUNT})
                const contributionFromContract = await crowdFundFunder.getContributions(HASH)
                assert.equal(MINIMUM_FUND_AMOUNT,contributionFromContract.toNumber())
            })
        })

        describe("campaign count",()=>{
            it("campaign count should be one",async()=>{
                const createTx = await createCampaign()
                const campaignCount = await crowdFund.getCampaignCount()
                assert.equal(campaignCount.toNumber(),1)
        ...
        describe("getHashById",()=>{
            it("hash from contract must be equal to Hash",async()=>{
                const createTx = await createCampaign()
                const campaign = await crowdFund.getCampaignByHash(HASH)
                const hashFromContract = await crowdFund.getHashById(campaign.id)
                assert.equal(hashFromContract,HASH)
